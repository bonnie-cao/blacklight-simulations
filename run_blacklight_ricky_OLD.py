...
def update_input_file(ifile, filename="example.input"):
  import re
  ...
  text = re.sub(
    r"(simulation_file\s*=\s*fm_torus\.out2\.)\d{5}(\.athdf)",
    r"\g<1>%05d\2" %ifile,
    text
  )

  text = re.sub(
        r"(output_file\s*=\s*\./output/dump_)\d+(\.npz)",
        rf"\g<1>%05d\2" %ifile,
        text
  )
...
def main():
    ...
    for ifile in indices:
        print(f"Processing dump {ifile}") 
        if (os.path.isfile("./output/dump_%05d.npz" %ifile)): continue
        update_input_file(ifile, filename="example_simulation.input")
